...
class Karel():
	...
	def openFrontLeft(self):
		# TODO 
		# Hint: It's very similar to openFarFront()
	...
	def forward(self):
		turtle.forward(BOX_SIZE)
		# TODO update x and y correctly
		# Hint: use a couple of if statements based on self.dir
		self.gather()
	...
	def gather(self): # note Arduino mod leaves negatives so need to fix that
		...
		if not self.openFrontLeft():
			# TODO update self.recon to include wall for front left
			# Hint look at how openFarFront is done
		if not self.openFrontRight():
			# TODO update self.recon to include wall for front right
	...
	def floodFillUpdate(self, toCenter): # some stacks, should optimize this, I believe it double counts a lot
		...
		while True:
			while currentLevel:
				...
				if cd == sys.maxsize:
					# TODO set current cell's map value to level
					# TODO append 
					# Hint: use getWalls() and isValid()
			if nextLevel:
				level += 1
				currentLevel = nextLevel
				nextLevel = []
	...
	def moveMin(self):
		...
		for j in range(4):
			if d[j] < d[i]:
				i = j
		# TODO i stores the direction we need to move in, so move in that direction
